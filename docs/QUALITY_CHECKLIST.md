...
For detailed troubleshooting of common issues, see:

📖 **[Common Pitfalls Database](COMMON_PITFALLS.md)**

This separate document contains:
- TypeScript/Node.js issues (Missing @types, shebang, ES modules, etc.)
- VitePress/Documentation issues (Base URL, missing pages, icon theming, etc.)
- Rust issues (Cargo.lock conflicts, CI zero-dep checks)
- CI/CD issues (Branch protection, authentication)
- General issues (Probabilistic tests, file-based state)
- Best practices (Dogfooding patterns)

**Quick reference:** When encountering an error, search the Common Pitfalls document by error message or symptom.
